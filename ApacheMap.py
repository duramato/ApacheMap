...
def find_lat_lng(ipaddr):
    ...
    if len(geo_value) > 0:
        ...
        point["properties"]["place"] = "{0}, {1}".format(geo_value.get('city', '').encode('ascii', 'ignore').decode('utf-8'), geo_value.get('country', '').encode('ascii', 'ignore').decode('utf-8'))
...
if __name__ == '__main__':
    if len(sys.argv) == 3:
        ...
        print("    ex: python ApacheMap.py add 0.0.0.0")
        sys.exit(1)
